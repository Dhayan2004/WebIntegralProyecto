...
def _is_strong_password(password: str) -> bool:
    return any(ch.isupper() for ch in password) and any(ch.isdigit() for ch in password)


class AuthService:
    @staticmethod
    def register(db: Session, name: str, email: str, password: str, organization_slug: str):
        if not _is_strong_password(password):
            raise ValueError("Password debe tener al menos una mayuscula y un digito")
        ...
        db.add(membership)
        db.add(UserProfile(user_id=user.id, preferences={"language": "es", "theme": "dark", "default_llm": "claude"}))
        ...
        db.add(
            AuthToken(
                ...
                expires_at=_utcnow() + get_email_verification_expire_timedelta(),
            )
        )
    ...
    @staticmethod
    def _get_membership(db: Session, user_id: str) -> OrganizationMember | None:
        return db.query(OrganizationMember).filter(OrganizationMember.user_id == user_id).first()

    @staticmethod
    def create_refresh_record(db: Session, user_id: str) -> str:
        ...
        db.add(
            AuthToken(
                ...
                token=refresh_token,
                token_type="refresh",
                expires_at=_utcnow() + get_refresh_token_expire_timedelta(),
            )
        )
        db.commit()
    ...
    @staticmethod
    def build_token_response(db: Session, user: User) -> dict:
        membership = AuthService._get_membership(db, user.id)
        organization_id = membership.organization_id if membership else None
        role = membership.role if membership else user.role
        refresh_token = AuthService.create_refresh_record(db, user.id)
        return {
            "access_token": create_access_token(user.id, organization_id=organization_id, role=role),
    ...
    @staticmethod
    def revoke_refresh_token(db: Session, refresh_token: str | None, user_id: str | None = None) -> None:
        query = db.query(AuthToken).filter(
            AuthToken.token_type == "refresh",
            AuthToken.revoked_at.is_(None),
        )
        if refresh_token:
            query = query.filter(AuthToken.token == refresh_token)
        elif user_id:
    ...
    def start_password_reset(db: Session, email: str) -> None:
        ...
        db.add(
            AuthToken(
                ...
                expires_at=_utcnow() + get_password_reset_expire_timedelta(),
            )
        )
        db.commit()
    ...
    def reset_password(db: Session, token: str, new_password: str) -> bool:
        ...
        AuthService.revoke_refresh_token(db, refresh_token=None, user_id=user.id)
        db.commit()
        return True
